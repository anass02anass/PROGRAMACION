...
public class Main3 {
	...
	public static void main(String[] args) throws SQLException, ClassNotFoundException {
		...
		// El m�todo getMetaData aplicado a un ResultSet nos devuelve un objeto
	    // de tipo ResultSetMetaData indic�ndonos el n�mero, tipo y propiedades 
	    // de las columnas que forman el ResultSet. Si le aplicamos adem�s el m�todo getColumnCount() 
	    // nos devuelve exactamente el n�mero de columnas  (8 en este caso)
	    
		for (int x=1;x<=rs.getMetaData().getColumnCount();x++)
				// Recorro cada una de las columnas para conocer su NOMBRE
				// getColumName(1) es el nombre  de la 1� columna (empnp_id en este caso)
				// getColumName(8) es el nombre de la 8� columna (dpto en este caso)...
		...
		System.out.println("------------");
		for (int x=1;x<=rs.getMetaData().getColumnCount();x++) {
			System.out.println(rs.getMetaData().getColumnName(x)+"-"+rs.getMetaData().getColumnTypeName(x)+"-"+rs.getMetaData().getColumnDisplaySize(x));
